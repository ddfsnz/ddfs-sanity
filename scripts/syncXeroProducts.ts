...
const run = async () => {
  ...
  const productsBefore = await sanityClient.fetch<number>(`count(*)`)
  console.log('Product count:', productsBefore)
  ...
  const productsAfter = await sanityClient.fetch<number>(`count(*)`)
  console.log('Product count:', productsAfter)
